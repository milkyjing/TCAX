...
void TcasFilter::ErrorInfo(const char *info) {
    char buf[1024];
    ::sprintf(buf, "%s - \"%s\"", info, m_filename);
...
HRESULT TcasFilter::InitializeIndex() {
    if (TCAS_FILE_TYPE_COMPRESSED == GETHI16B(m_header.flag)) {
        if (::libtcas_create_ordered_index(&m_file, &m_header, 0, 0, &m_pOrderedIndexs, NULL) != tcas_error_success) {
            ErrorInfo("Failed to create the TCAS index");
            return E_FAIL;
        }
        if ((m_indexStreams = ::libtcas_linearize_ordered_indexs(m_pOrderedIndexs, m_header.chunks, NULL)) == NULL) {
            ErrorInfo("Failed to linearize the TCAS index");
            return E_FAIL;
        }
        ::free(m_pOrderedIndexs);
        m_pOrderedIndexs = NULL;
...
HRESULT TcasFilter::Initialize(const CMediaType *mtIn) {
    if (!m_isTcasInitialized) {
        ...
        m_fpsNumerator = 10000000;
        m_fpsDenominator = pvi->AvgTimePerFrame;
        if (::libtcas_frame_cache_init(m_filename, m_fpsNumerator, m_fpsDenominator, width, height, TCAS_MAX_FRAME_CACHED, TCAS_DEFAULT_FILE_CACHE_SIZE, &m_frameCache) != tcas_error_success) {
            ErrorInfo("Error: failed to initialize the frame cache caused by access failure to the TCAS file.");
            return VFW_E_TYPE_NOT_ACCEPTED;
        }
        ...
        m_minTime = CRefTime((long)m_frameCache.fcpArgs.header.minTime);
        m_maxTime = CRefTime((long)m_frameCache.fcpArgs.header.maxTime);
...
void TcasFilter::RenderRGB24(IMediaSample *pSample, VIDEOINFOHEADER *pvi, CRefTime &tStart) {
    ...
    int n = (int)((ULONGLONG)tStart.Millisecs() * m_fpsNumerator / (m_fpsDenominator * 1000));
