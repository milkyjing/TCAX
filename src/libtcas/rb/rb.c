...
rb_entry rbtree_find(const RbTreePtr tree, const rb_entry key) {
    int res;
    RbNodePtr p;
    p = tree->root;
    while (p) {
...
rb_entry *rbtree_probe(RbTreePtr tree, const rb_entry item) {
...
int rbtree_insert(RbTreePtr tree, const rb_entry item) {
    rb_entry *p = rbtree_probe(tree, item);
    return (p == NULL) ? -1 : 0;
}

int rbtree_delete(RbTreePtr tree, const rb_entry key) {
    RbNodePtr pa[RB_MAX_HEIGHT];        /* Nodes on stack. */
    unsigned char da[RB_MAX_HEIGHT];    /* Directions moved from stack nodes. */
