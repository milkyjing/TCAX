...
typedef void *rb_entry;        /* type of the data stored in the node */
...
typedef struct _rb_node {
    ...
    rb_entry data;                  /* Pointer to data. */
    unsigned char color;            /* Color. */
} RbNode, *RbNodePtr;

typedef int (*rb_compare_func)(const rb_entry, const rb_entry, const void *);     /* this function is very important, and should be specifed */
typedef rb_entry (*rb_copy_func)(const rb_entry, const void *);
typedef void (*rb_cleanup_func)(rb_entry, const void *);    /* the parameter passed to clean_f is the address of the ith element */
...
extern rb_entry rbtree_find(const RbTreePtr tree, const rb_entry key);

extern rb_entry *rbtree_probe(RbTreePtr tree, const rb_entry item);

extern int rbtree_insert(RbTreePtr tree, const rb_entry item);

extern int rbtree_delete(RbTreePtr tree, const rb_entry key);
