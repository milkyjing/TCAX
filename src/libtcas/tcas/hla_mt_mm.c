...
void libtcas_get_specified_chunk(const tcas_unit *fileMapBuf, tcas_u32 offset, TCAS_pChunk pChunk) {
    tcas_u32 count;
    memcpy(pChunk, fileMapBuf + offset, 3 * sizeof(tcas_unit));  /* startTime endTime cltp takes up 3 tcas_unit */
    ...
    memcpy(pChunk->pos_and_color, fileMapBuf + offset + 3, count * (sizeof(tcas_unit) << 1));
}

TCAS_Error_Code libtcas_create_ordered_index_mm(const tcas_unit *fileMapBuf, const TCAS_pHeader pHeader, tcas_u32 fpsNumerator, tcas_u32 fpsDenominator, TCAS_pOrderedIndex *ppOrderedIndexs, tcas_u32 *pOrderedIndexCount) {
    TCAS_pOrderedIndex pOrderedIndexs;
    tcas_u32 i, offset, chunks;
    if (TCAS_FILE_TYPE_COMPRESSED != GETHI16B(pHeader->flag)) return tcas_error_file_type_not_match;
    ...
    if (0 == fpsNumerator || 0 == fpsDenominator) {
        for (i = 0; i < chunks; i++) {
            pOrderedIndexs[i].first = fileMapBuf[offset + 0];    /* startTime */
            pOrderedIndexs[i].last = fileMapBuf[offset + 1];     /* endTime */
            if (pOrderedIndexs[i].first == pOrderedIndexs[i].last && 0 == GETTYPE(fileMapBuf[offset + 2]))  /* a normal chunk which has no effect */
                pOrderedIndexs[i].id_pair = MAKEINDEXIP(0, 1);  /* will not be used */
            else
                pOrderedIndexs[i].id_pair = MAKEINDEXIP(i + 1, GETPAIR(fileMapBuf[offset + 2]));            /* used to reserve the order of chunks, note that valid id starts from 1 */
            pOrderedIndexs[i].offset = offset;
            offset += 3 + (GETCOUNT(fileMapBuf[offset + 2]) << 1);
        }
    } else {
        for (i = 0; i < chunks; i++) {
            pOrderedIndexs[i].first = (tcas_u32)((tcas_u64)fileMapBuf[offset + 0] * fpsNumerator / (fpsDenominator * 1000)) + 1;    /* firstFrame, note: +1 is just intend to make it compatible with VSFilter */
            pOrderedIndexs[i].last = (tcas_u32)((tcas_u64)fileMapBuf[offset + 1] * fpsNumerator / (fpsDenominator * 1000)) + 1;    /* lastFrame, note: +1 is just intend to make it compatible with VSFilter */
            if (pOrderedIndexs[i].first == pOrderedIndexs[i].last && 0 == GETTYPE(fileMapBuf[offset + 2]))  /* a normal chunk which has no effect */
                pOrderedIndexs[i].id_pair = MAKEINDEXIP(0, 1);  /* will not be used */
            else
                pOrderedIndexs[i].id_pair = MAKEINDEXIP(i + 1, GETPAIR(fileMapBuf[offset + 2]));            /* used to reserve the order of chunks */
            pOrderedIndexs[i].offset = offset;
            offset += 3 + (GETCOUNT(fileMapBuf[offset + 2]) << 1);
        }
    }
...
static void *_work_parse_linearized_index_streams_normal_mt_mm(void *args) {
    ...
    fileMapBuf = (tcas_unit *)((tcas_p32)args)[0];  /* get pFile from args */
    ...
    i = (tcas_u32)((tcas_p32)args)[4];  /* get i from args */
    ...
    if (pOrderedIndexLast->first <= n && n < pOrderedIndexLast->last) {  /* last chunk is still valid */
        if (NULL == pChunksCache->pChunks[i].pos_and_color) {  /* not read yet, this may happen just after the initialization */
            libtcas_get_specified_chunk(fileMapBuf, pOrderedIndexLast->offset, &pChunksCache->pChunks[i]);  /* otherwise, we do not have to read it again */
        ...
        if (pChunksCache->pi[i] + 1 < indexCount) {  /* there is still a next index */
            ...
            if (pOrderedIndexNext->first <= n && n < pOrderedIndexNext->last) {  /* the next index is valid */
                ...
                libtcas_get_specified_chunk(fileMapBuf, pOrderedIndexNext->offset, &pChunksCache->pChunks[i]);  /* cover the last one */
    ...
    if (TCAS_FALSE == found) {  /* we cannot find the index easily (maybe the video is moved forward or backward), so we use binary search to try again */
        ...
        if (-1 != result) {  /* we find it */
            ...
            if (pChunksCache->pChunks[i].pos_and_color)
                free(pChunksCache->pChunks[i].pos_and_color);  /* free the previous chunk */
            libtcas_get_specified_chunk(fileMapBuf, pOrderedIndexLast->offset, &pChunksCache->pChunks[i]);  /* cover the last one */
            pChunksCache->pChunks[i].startTime = GETINDEXID(pOrderedIndexLast->id_pair);  /* note that with the help of indexs startTime and endTime are no longer needed, we can use them to do some other business */
            pChunksCache->pChunks[i].endTime = GETLAYER(pChunksCache->pChunks[i].cltp);
            found = TCAS_TRUE;
            return (void *)0;
...
static void _libtcas_parse_linearized_index_streams_normal_mt_mm(tcas_unit *fileMapBuf, const TCAS_IndexStreamsPtr indexStreams, tcas_u32 n, TCAS_pChunksCache pChunksCache, ThreadpoolPtr tp) {
    tcas_u32 i, size, count;
    ThreadpoolWork work;
    count = vector_get_count(indexStreams);
    size = 5 * sizeof(tcas_p32);  /* there are 6 parameters */
    work.execution_route = (tpw_func)_work_parse_linearized_index_streams_normal_mt_mm;
    work.args_deepcopy = 1;  /* set args_deepcopy as true */
    for (i = 0; i < count; i++) {    /* check every index stream */
        work.args = malloc(size);
        ((tcas_p32 *)work.args)[0] = (tcas_p32)fileMapBuf;
        ((tcas_p32 *)work.args)[1] = (tcas_p32)indexStreams;
        ((tcas_p32 *)work.args)[2] = (tcas_p32)n;
        ((tcas_p32 *)work.args)[3] = (tcas_p32)pChunksCache;
        ((tcas_p32 *)work.args)[4] = (tcas_p32)i;
...
static void *_work_parse_linearized_index_streams_key_mt_mm(void *args) {
    /* variables come directly from _libtcas_parse_linearized_index_streams_normal_mt function parameters */
    tcas_unit *fileMapBuf;
    TCAS_IndexStreamsPtr indexStreams;
    tcas_u32 n;
    ...
    if (pOrderedIndexLastStart->first == pOrderedIndexLastStart->last && 0 != GETINDEXID(pOrderedIndexLastStart->id_pair) && 0 == GETINDEXPAIR(pOrderedIndexLastStart->id_pair)) {  /* the last chunk is a key frame chunks */
        ...
        if (pOrderedIndexLastStart->first <= n && n < pOrderedIndexLastEnd->last) {  /* last chunk is still valid, note that, the last frame cannot be reached in order to be compatible with normal chunk frames */
            if (NULL == pChunksCache->pChunkPairs[i].start.pos_and_color) {  /* not read yet, this may happen just after the initialization, it is impossible that only one pos_and_color is NULL, must be both or neither */
                libtcas_get_specified_chunk(fileMapBuf, pOrderedIndexLastStart->offset, &pChunksCache->pChunkPairs[i].start);  /* otherwise, we do not have to read it again */
                libtcas_get_specified_chunk(fileMapBuf, pOrderedIndexLastEnd->offset, &pChunksCache->pChunkPairs[i].end);
            ...
            if (pChunksCache->pi[i] + 3 < indexCount) {  /* there may still be a key frame chunk index next to the two key frame chunks */
                ...
                if (pOrderedIndexNextStart->first == pOrderedIndexNextStart->last && 0 != GETINDEXID(pOrderedIndexNextStart->id_pair) && 0 == GETINDEXPAIR(pOrderedIndexNextStart->id_pair)) {  /* is key frame chunk */
                    ...
                    if (pOrderedIndexNextStart->first <= n && n < pOrderedIndexNextEnd->last) {  /* last chunk is still valid, note that, the last frame cannot be reached in order to be compatible with normal chunk frames */
                        ...
                        libtcas_get_specified_chunk(fileMapBuf, pOrderedIndexNextStart->offset, &pChunksCache->pChunkPairs[i].start);
                        libtcas_get_specified_chunk(fileMapBuf, pOrderedIndexNextEnd->offset, &pChunksCache->pChunkPairs[i].end);
                        libtcas_key_frame_chunk_add_dips(&pChunksCache->pChunkPairs[i].start, &pChunksCache->pChunkPairs[i].end);
                        if (1 == GETTYPE(pChunksCache->pChunkPairs[i].start.cltp))
    ...
    if (TCAS_FALSE == found) {  /* we cannot find the index easily (maybe the video is moved forward or backward), so we use binary search to try again */
        ...
        if (-1 != result) {
            ...
            libtcas_get_specified_chunk(fileMapBuf, pOrderedIndexLastStart->offset, &pChunksCache->pChunkPairs[i].start);  /* cover the last one */
            libtcas_get_specified_chunk(fileMapBuf, pOrderedIndexLastEnd->offset, &pChunksCache->pChunkPairs[i].end);  /* cover the last one */
            libtcas_key_frame_chunk_add_dips(&pChunksCache->pChunkPairs[i].start, &pChunksCache->pChunkPairs[i].end);
            if (1 == GETTYPE(pChunksCache->pChunkPairs[i].start.cltp))
                libtcas_generate_intermediate_frame_chunk_fast(&pChunksCache->pChunkPairs[i].start, &pChunksCache->pChunkPairs[i].end, pOrderedIndexLastStart->first, pOrderedIndexLastEnd->last, n, &pInterChunkTemp);
            else if (2 == GETTYPE(pChunksCache->pChunkPairs[i].start.cltp))
...
static void _libtcas_parse_linearized_index_streams_key_mt_mm(tcas_unit *fileMapBuf, const TCAS_IndexStreamsPtr indexStreams, tcas_u32 n, TCAS_pChunksCache pChunksCache, ThreadpoolPtr tp) {
    tcas_u32 i, size, count;
    ThreadpoolWork work;
    count = vector_get_count(indexStreams);
    size = 5 * sizeof(tcas_p32);  /* there are 6 parameters */
    work.execution_route = (tpw_func)_work_parse_linearized_index_streams_key_mt_mm;
    work.args_deepcopy = 1;  /* set args_deepcopy as true */
    for (i = 0; i < count; i++) {    /* check every index stream */
        work.args = malloc(size);
        ((tcas_p32 *)work.args)[0] = (tcas_p32)fileMapBuf;
...
TCAS_Error_Code libtcas_create_frame_with_linearized_index_mt_mm(tcas_unit *fileMapBuf, const TCAS_pHeader pHeader, const TCAS_IndexStreamsPtr indexStreams, tcas_u32 n, TCAS_pChunksCache pChunksCache, ThreadpoolPtr tp, tcas_u16 targetWidth, tcas_u16 targetHeight, tcas_byte **pBuf) {
    ...
    _libtcas_parse_linearized_index_streams_normal_mt_mm(fileMapBuf, indexStreams, n, pChunksCache, tp);
    if (keyframing)
        _libtcas_parse_linearized_index_streams_key_mt_mm(fileMapBuf, indexStreams, n, pChunksCache, tp);
