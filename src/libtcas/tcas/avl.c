...
avl_entry_ptr avltree_find(const AvlTreePtr tree, const avl_entry_ptr key) {
    int res;
    AvlNodePtr p;
    p = tree->root;
    while (p) {
...
avl_entry_ptr *avltree_probe(AvlTreePtr tree, const avl_entry_ptr item) {
...
int avltree_insert(AvlTreePtr tree, const avl_entry_ptr item) {
    avl_entry_ptr *p = avltree_probe(tree, item);
    return (p == NULL) ? -1 : 0;
}

int avltree_delete(AvlTreePtr tree, const avl_entry_ptr key) {
