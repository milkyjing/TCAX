/*
 * hla_double_cache.h -- High-level double cache API of the 'libtcas' library
...
#ifndef LIBTCAS_HLA_DOUBLE_CACHE_H
#define LIBTCAS_HLA_DOUBLE_CACHE_H
#pragma once
 ...
 */
typedef struct _tcas_double_cache_proc_args {
    TCAS_FrameChunksCache fcc;
    tcas_u32 n;    /* should be specified during the rendering time, not in the initialization */
    tcas_u16 width;
    tcas_u16 height;
} TCAS_DoubleCacheProcArgs, *TCAS_pDoubleCacheProcArgs;
...
typedef struct _tcas_frame_cache {
    ...
    DWORD threadID;
    int active;
    TCAS_DoubleCacheProcArgs dcpArgs;
    tcas_u32 minFrame;
    tcas_u32 maxFrame;
} TCAS_DoubleCache, *TCAS_pDoubleCache;
 ...
 */
extern TCAS_Error_Code libtcas_double_cache_init(const char *filename, tcas_u32 fpsNumerator, tcas_u32 fpsDenominator, tcas_u16 width, tcas_u16 height, int maxFrameCount, int maxFrameChunkCount, int fileCacheSize, TCAS_pDoubleCache pDoubleCache);
 ...
 */
extern void libtcas_double_cache_run(TCAS_pDoubleCache pDoubleCache);
 ...
 */
extern int libtcas_double_cache_get(TCAS_pDoubleCache pDoubleCache, tcas_u32 n, tcas_byte **pBuf);

/**
 * Finalize the frame cache facility.
 * @param pFrameCache a pointer to TCAS_FrameCache structure, which should have been initialized
 */
extern void libtcas_double_cache_fin(TCAS_pDoubleCache pDoubleCache);

#ifdef __cplusplus
}
#endif    /* __cplusplus */

#endif    /* LIBTCAS_HLA_DOUBLE_CACHE_H */
