...
typedef void *avl_entry;        /* type of the data stored in the node */
...
typedef int (*avl_compare_func)(const avl_entry, const avl_entry, const void *);     /* this function is very important, and should be specifed */
typedef avl_entry (*avl_copy_func)(const avl_entry, const void *);
typedef void (*avl_cleanup_func)(avl_entry, const void *);    /* the parameter passed to clean_f is the address of the ith element */
...
extern avl_entry avltree_find(const AvlTreePtr tree, const avl_entry key);

extern avl_entry *avltree_probe(AvlTreePtr tree, const avl_entry item);

extern int avltree_insert(AvlTreePtr tree, const avl_entry item);

extern int avltree_delete(AvlTreePtr tree, const avl_entry key);
