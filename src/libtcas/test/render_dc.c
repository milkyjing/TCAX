#include "../tcas/hla_double_cache.h"
#include "../tcas/tcas.c"
#include "../tcas/hla_chunk_cache.c"
#include "../tcas/hla_double_cache.c"
...
TCAS_DoubleCache g_doubleCache;
...
int tcas_renderer_init(const char *filename, double fps) {
    ...
    g_width = 1280;
    g_height = 720;
    libtcas_double_cache_init(filename, g_fpsNumerator, g_fpsDenominator, g_width, g_height, TCAS_MAX_FRAME_CACHED, TCAS_MAX_FRAME_CHUNKS_CACHED, &g_doubleCache);
    //g_frameCache.fcpArgs.fileCache.temp = 0;
    libtcas_double_cache_run(&g_doubleCache);
    return 0;
}

void tcas_renderer_fin() {
    libtcas_double_cache_fin(&g_doubleCache);
}
...
void tcas_render_rgb32(int n) {
    ...
    // Create TCAS frame
    while (!libtcas_double_cache_get(&g_doubleCache, n, &tcasFrameBuf))
        continue;
    // Render
...
int main() {
    ...
    count = 200;
    t0 = GetTickCount();
    for (i = g_doubleCache.minFrame; i < g_doubleCache.maxFrame; i++) {
        //t1 = GetTickCount();
        tcas_render_rgb32(i);
        //t2 = GetTickCount();
    ...
    t3 = GetTickCount();
    tcas_renderer_fin();
    printf("\ntotal time for rendering %i frames %i\n", g_doubleCache.maxFrame - g_doubleCache.minFrame, t3 - t0);
