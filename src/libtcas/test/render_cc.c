...
int tcas_renderer_init(const char *filename, double fps) {
    ...
    g_width = 1280;
    g_height = 720;
    libtcas_frame_chunks_cache_init(filename, g_fpsNumerator, g_fpsDenominator, g_width, g_height, TCAS_MAX_FRAME_CHUNKS_CACHED, &g_fcc);
    libtcas_frame_chunks_cache_run(&g_fcc);
    return 0;
}

void tcas_renderer_fin() {
    libtcas_frame_chunks_cache_fin(&g_fcc);
}
...
void tcas_render_rgb32(int n) {
    ...
    // Create TCAS frame
    while (!libtcas_frame_chunks_cache_get(&g_fcc, n, &vFrameChunks))
        continue;
    libtcas_create_frame_with_chunks_cached(&g_fcc, vFrameChunks, width, height, &tcasFrameBuf);
    // Render
...
int main() {
    ...
    if (tcas_renderer_init("E:\\杂项\\fuck1.tcas", 25) != 0) {
    ...
    count = 200;
    t0 = GetTickCount();
    for (i = g_fcc.minFrame; i < g_fcc.maxFrame; i++) {
        //t1 = GetTickCount();
        tcas_render_rgb32(i);
        //t2 = GetTickCount();
    ...
    t3 = GetTickCount();
    tcas_renderer_fin();
    printf("\ntotal time for rendering %i frames %i\n", g_fcc.maxFrame - g_fcc.minFrame, t3 - t0);
